        ...
        <el-table :data="tableData" style="width: 100%; margin-top: 25px;">
            ...
            <el-table-column label="用户" width="80" >
                <template #default="scope">
                    <el-avatar style="background-color: #409EFF; font-size: 14px;">
            ...
            <el-table-column label="生活指标" width="120" >
                <template #default="scope">
                    <div>
                        <p style="margin: 0; line-height: 1.5;">睡眠：{{ scope.row.sleepQuality }} / 5</p>
                        <p style="margin: 0; line-height: 1.5;">压力：{{ scope.row.stressLevel }} / 5</p>
                    </div>
                </template>
            </el-table-column>
            
            <el-table-column prop="emotionTriggers" label="情绪触发因素" width="120" />
            <el-table-column prop="diaryContent" label="日记内容" width="250" show-overflow-tooltip />
                ...
                <div class="detail-section">
                    ...
                    <div class="ai-analysis-result">
                        ...
                        <el-descriptions-item label="情绪强度">
                            <el-progress :percentage="aiData.emotionScore" :color="getEmotionScoreColor" :stroke-width="8"/>
                        </el-descriptions-item>
                        <el-descriptions-item label="风险等级">
                            <el-tag :type="getAiEmotionTagType(aiData.riskLevel)">{{ aiData.riskLevel }}</el-tag>
                        </el-descriptions-item>
                        <el-descriptions-item label="情绪性质">
                            <el-tag :type="aiData.isNegative ? 'danger' : 'success'">{{ aiData.riskLevel ? '负面情绪' : '正面情绪' }}</el-tag>
                        </el-descriptions-item>
                    </el-descriptions>
...
import { ElMessageBox, ElMessage } from 'element-plus' // 补充引入 ElMessage
...
// 修复2：赋默认值 formData = {}，防止 onMounted 时传 undefined 报错
const handleSearch = async (formData = {}) => {
    const params = {
        page: pagination.currentPage,
        size: pagination.size,
        ...formData // 修复2：大写 FormData 改为小写 formData
    }
    ...
    try {
        ...
        tableData.value = actualData.records || []
...
const currentDetail = ref(null) // 修复1：{null} 改为 null
...
const handleDelete = (row) => {
    ElMessageBox.confirm('确定删除该条记录吗？', '删除确认', {
        ...
        cancelButtonText: '取消',
        type: 'warning' // 修复：denger 改为 warning
    }).then(() => {
        //确认删除
        deleteEmotional(row.id).then(() => {
            ElMessage.success('删除成功') // 加上这行，交互体验更好
            handleSearch()
        })
    }).catch(() => {
        //取消删除
