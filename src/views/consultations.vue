<template>
    <div>
        ...
        <el-table :data="tableData" style="width: 100%;">
            <el-table-column label="用户头像" width="100">
                <template #default="scope">
                    <el-avatar style="background-color: #409EFF;">
                        {{ scope.row.userNickname ? scope.row.userNickname.substring(0, 1) : '用' }}
                    </el-avatar>
                </template>
            </el-table-column>
            ...
            <el-table-column prop="lastMessageTime" label="创建时间" width="200"/>
...
// 修复：sessionDatail 拼写错误修正为 sessionDetail
...
const viewSessionDetail = (row) => {
    ...
    getSessionDetail(row.id).then(res => {
        ...
        // 适配拦截器，直接取 res，如果还有嵌套可能是 res.data
